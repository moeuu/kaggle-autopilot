...
- ✅ **Data-free kernel contract verification**: Generated kernels compile and export `contract_smoke()`, which Kagglebot calls in an isolated subprocess to validate every frozen pipeline without training or scoring. For explicit writeup deliverables or frozen plans that require local training, Kagglebot treats `dataset_profile_missing_required_files` as informational after a valid data-free smoke, then records a resumable data blocker before training. Other prediction routes retain the training-data readiness gate. Contract-only verification creates no submission/OOF/checkpoint artifacts during the smoke and can resume when the declared inputs are available.
...
- ✅ **Automated judged Writeups**: writeup competitions resolve explicitly required notebook outputs from the persisted plan and official competition context, so artifacts such as `features.csv` are validated by name instead of being rejected as non-submissions. When a private notebook is required, submission-enabled `--force` runs publish it, download and hash-check the required outputs against the locally validated artifacts, add the private notebook link to the report, then check participation/rules and submit through Kaggle's authenticated Projects/Writeup UI. The complete report/notebook/artifact identity is recorded and duplicate or ambiguous attempts are never retried automatically.
