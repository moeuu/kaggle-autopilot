...
campaign-aware submission message resolution/file-submit-to-notebook fallback decisions live in
...
4. Submit state persistence: submit attempt JSONL writing/reading, duplicate SHA lookup, submit attempt/run-state payloads,
   ...
   outcome abort decisions, campaign-aware submission message resolution, and file-submit-to-notebook fallback decisions
   ...
   `submit_failure_context` modules rather than adding more private wrappers in `autopilot.py`.
