...
Bug-like loop stagnation decisions such as no-improvement patience and repeated same-config stops live in
...
5. **Compatibility wrapper retirement**: after call sites move to extracted modules, remove private wrappers in
   ...
   Loop stagnation stop-reason construction now calls `loop_control.py` directly.
