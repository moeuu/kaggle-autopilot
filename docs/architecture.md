...
2. Submission decision policy: keep moving candidate quality holdback, forced-submit reasons, and submit deferral into
   ...
   resolvers, and public `run_state.json` load/save helpers now live in `autopilot_state.py`, so orchestration code no
   longer open-codes state file paths when writing lifecycle status, resolving iteration artifacts, iteration completion
   markers, summaries, or submit state.
