...
- [ ] Planning order remains `codex -> oracle(latest-pro) -> codex`; an unavailable, failed, timed-out, or empty Oracle response preserves diagnostics and falls back to `gpt-5.6-sol` with xhigh reasoning, while structurally invalid strategy output still fails the normal quality gates.
- [ ] Only the Codex implementation after a valid Oracle response uses the shared `sol-xhigh` profile; pre-Oracle brief extraction remains on the normal profile.
